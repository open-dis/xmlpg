...
/**
 * Represents an initial value for a primitive type. This can be set in a subclass of a given class, for example to
 * initialize a header, like version number of packet type.
 *
 * @author DMcG
 */
public class InitialValue {
    ...
    public String getSetterMethodNameCSharp() // PES added for CSharp Support
    {
        ...
        // this caused problems as it was already capitialized and the get/set method would have had the same name.
        // Therefore
        // I decided to append the underscore "_" to the protected variable names, this worked except for the following
        // problem.
        // In two instances the Class name was the same name as one of its fields except that the first letter was cap
        // which
        // caused the accessor methods to return an error stating member names can not be the same as
        // there enclosing type. Anthore problem was a key word was used "System". Therefore my solution was to check
        // for the class name and the field names being the same and changing
        // the field name by adding an underscore at the end which is being done in the CSharpGenerator.java file, this
