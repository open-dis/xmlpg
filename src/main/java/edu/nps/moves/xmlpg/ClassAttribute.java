...
/**
 * Represents one attribute of a class, ie an instance variable. this may be a primitive type, a class defined elsewhere
 * in the document, a list, or an array.
 *
 * @author DMcG
 */
public class ClassAttribute {
    ...
    /**
     * The various things an attribute can be: a primitive type (int, short, byte, etc), a reference to another class
     * defined in this document, a list of fixed length, aka an array, or a list of variable length,
     */
    public enum ClassAttributeType {
        UNSET, PRIMITIVE, CLASSREF, FIXED_LIST, VARIABLE_LIST
    };
    ...
    /**
     * If this is a variable list length field, when unmarshalling we need to know how many to unmarshal. This is the
     * name of the filed that contains that count.
     */
    protected String countFieldName;

    /**
     * If this is a primitive type (specifically some sort of integer) we may be the count field for a variable length
     * list. If that's the case, the getter method is different, and there is no setter method. Instead we simply use
     * the current actual length of the dynamic list.
     */
    protected boolean isDynamicListLengthField = false;

    /**
     * If this is a dynamic length list field, we also need the field that this tells the length for.
     */
    ...
    /**
     * If this is a list of some sort, this is true if the list consists of primitives, false if the list consists of
     * classes
     */
    protected boolean underlyingTypeIsPrimitive = false;

    /**
     * If this is a list of some sort, this is true if the list consists of class references, false if the list consists
     * of primitives
     */
    protected boolean underlyingTypeIsClass = false;

    /**
     * Some fields, such as Marking, could have arrays that are treated a C strings. At least on the set method, if we
     * pass in an array we can have an alternate method that treats the input string as a c-style string, with a
     * terminating null character. This is not strictly compliant with the DIS standard, which makes no assumptions
     * about null-terminated strings, but it happens often enough in the C world to special case it.
     */
    ...
    /**
     * Should we serialize this attribute to the message or not? By default yes, but this can be overridden by the
     * attribute serialize="false" in the xml
     */
    ...
    public boolean listIsClass() {
        if (!((attributeKind == ClassAttributeType.FIXED_LIST)
                || (attributeKind == ClassAttributeType.VARIABLE_LIST))) {
            return false;
        }
